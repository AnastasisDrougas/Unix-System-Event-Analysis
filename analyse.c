...
void file_checking(int argc, char *argv[]){
    ...
    if(argc < 2){
        ...
        printf("\nYou must give one file at a time!\nUsage: %s <YourLogFile>\n", argv[0]);
        printf("-------------------------------------------\n\n");
        exit(EXIT_FAILURE);
    }
    ...
    if(strlen(argv[1]) >= 4 && strcmp(argv[1] + strlen(argv[1]) - 4, ".log") == 0){
        ...
        printf("Incorrect file type..\nThe given file is not a '.log' file!\n");
        printf("-------------------------------------------\n\n");
        exit(EXIT_FAILURE);
    }
}
...
int extract_data(int fd){
    ...
    return print_and_return_retval(total_lines, total_errors, total_digits);
}
